...
const wchar_t* D3D12MeshletRender::c_meshFilename = L"..\\Assets\\Flower.bin";
const wchar_t* c_texturePath = L"..\\Assets\\Flower.png";
...
{
    m_camera.Init({ 0, -5, 30 });  // Камера ниже по Y и ближе по Z
    m_camera.SetMoveSpeed(50.0f);  // Скорость перемещения уменьшена
...
void D3D12MeshletRender::OnUpdate()
{
    ...
    XMMATRIX world = XMMatrixScaling(1.0f, 1.0f, 1.0f) * XMMatrixTranslation(0.0f, 0.0f, 0.0f);
     // Поворачиваем цветок на 180 градусов вокруг оси Y
    //XMMATRIX world = XMMatrixScaling(1.0f, 1.0f, 1.0f) *
    //    XMMatrixRotationY(XM_PI) *  // Поворот на 180 градусов
