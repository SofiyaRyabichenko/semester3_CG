...
const wchar_t* D3D12MeshletRender::c_meshFilename = L"..\\Assets\\ARTIST.bin";
...
D3D12MeshletRender::D3D12MeshletRender(UINT width, UINT height, std::wstring name)
    ...
    , m_fenceEvent{}
    , m_fenceValues{}
    , m_mouseLeftPressed(false)        // Добавлено
    , m_lastMouseX(0)                  // Добавлено
    , m_lastMouseY(0)                   // Добавлено
    , m_mouseSensitivity(0.005f)        // Добавлено
{ }
...
void D3D12MeshletRender::OnInit()
{
    ...
    m_camera.SetMouseSensitivity(m_mouseSensitivity);  // Добавлено

    LoadPipeline();
    LoadAssets();
}

void D3D12MeshletRender::OnMouseDown(WPARAM buttonState, int x, int y)
{
    if (buttonState & MK_LBUTTON)
    {
        ...
        // Можно скрыть курсор для более плавного управления
        ShowCursor(FALSE);
    }
}

void D3D12MeshletRender::OnMouseUp(WPARAM buttonState, int x, int y)
{
    if (!(buttonState & MK_LBUTTON))
    {
        m_mouseLeftPressed = false;

        // Показать курсор обратно
        ShowCursor(TRUE);
    }
}

void D3D12MeshletRender::OnMouseMove(WPARAM buttonState, int x, int y)
{
    if (m_mouseLeftPressed)
    {
        ...
        // Передаем дельту в камеру (нужно будет добавить соответствующий метод в SimpleCamera)
        // Пока будем обновлять камеру напрямую через её поля
        // Но лучше добавить метод в SimpleCamera для обработки мыши

        // Передаем дельты напрямую в камеру
        m_camera.OnMouseMove(deltaX, deltaY, true);
